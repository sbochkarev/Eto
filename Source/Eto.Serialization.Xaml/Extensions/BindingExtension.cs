	...
	public class BindingExtension : MarkupExtension
	{
		...
		public override object ProvideValue(IServiceProvider serviceProvider)
		{
			...
			if (pti.IsGenericType && pti.GetGenericTypeDefinition() == typeof(IIndirectBinding<>))
			{
				if (Converter != null)
				{
					var instance = Binding.Property<object>(Path).Convert(Converter, ConverterParameter, ConverterCulture);
					var cast = instance.GetType().GetRuntimeMethod("Cast", new Type[0]);
					var castMethod = cast.MakeGenericMethod(pti.GenericTypeArguments);
					return castMethod.Invoke(instance, null);
				}
			...
			if (widget != null && !typeof(IBinding).GetTypeInfo().IsAssignableFrom(propertyType.GetTypeInfo()))
			{
				...
				if (Converter != null)
					sourceBinding = sourceBinding.Convert(Converter, ConverterParameter, ConverterCulture);

				var binding = widget.BindDataContext<object>(Binding.Property<object>(propertyInfo.Name), sourceBinding, Mode);
